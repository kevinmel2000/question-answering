...
def getNamedEntities(path):
    ...
    locations = map(lambda x: x.lower(),list(set(filter(lambda x: x != None, reduce(lambda y,z: y+z,map(lambda line: re.findall('<ENAMEX TYPE="LOCATION">([^<>]*)</ENAMEX>',line),lines))))))
    persons = map(lambda x: x.lower(),list(set(filter(lambda x: x != None, reduce(lambda y,z: y+z,map(lambda line: re.findall('<ENAMEX TYPE="PERSON">([^<>]*)</ENAMEX>',line),lines)+map(lambda line: re.findall('<ENAMEX TYPE="ORGANIZATION">([^<>]*)</ENAMEX>',line),lines))))))
    times = map(lambda x: x.lower(),list(set(filter(lambda x: x != None, reduce(lambda y,z: y+z,map(lambda line: re.findall('<TIMEX TYPE="DATE">([^<>]*)</TIMEX>',line),lines))))))
...
def processQuestion(question,corefre1,corefre2,corefre3,corefre4,corefre5,corefre6,weights,locations,persons,times):
    ids=dict();
    text=question[0];
    corrections={'Mr.':'Mr','Dr.':'Dr','Oct.':'Oct','Mrs.':'Mrs','St.':'St','ST.':'ST','P.T.':'PT','MT.':'MT','Mt.':'Mt','A.D.':'AD'};
    text=plain(text);
...
def similarity(question, idf, constraints, sentence, entities):
    ...
    return score*getScore(constraints, sentence[0], entities)# - len(sentence[0])/2
...
def getBestAnswer(question, constraints, sentences, entities):
    ...
    score[bestAnswer] = 0
    secondBest = score.index(max(score))
    
    return (sentences[bestAnswer][0], sentences[secondBest][0])
...
weights4={'WRB': 0.39628601626111559, 'PRP$': 0.71584275114738904, 'VBG': 0.41544848640434506,
          'VBD': 1.4762872745169731, 'CC': 0.63750733211878219, 'IN': 1.4972648780578646,
          'VBN': 1.2799632527744693, 'POS': 0.39628601626111559, '-NONE-': 0, 'VBP': 1.3184399912206954,
          'WDT': -0.23174647487149924, 'JJ': 1.8127739578666003, 'WP': -2.3025850929940455, 'VBZ': 1.4337370850951192,
          'DT': 0, 'CD': 1.1872499018914846, 'MD': 1.2225502301940836, 'NNPS': 0.21584275114738904, 'RP': 0.89628601626111559,
          '$': -30.3025850929940455, 'NN': 1.7035887925742386, 'RBR': 1.23174647487149924, 'JJS': 0.18116446150742038, ',': 0,
          '.': 0, 'VB': 1.379068570587922, 'TO': 2.2961913175294344, 'PRP': 1.7433749904311804, 'RB': 0.9801850546085602,
          ':': -2.3025850929940455, 'NNS': 1.3737529457364159, 'JJR': -1.3025850929940455, 'NNP': 0.4186174249648332}

weights5={'WRB': 0.39, 'PRP$': 0.71, 'VBG': 0.41,
          'VBD': 1.47, 'CC': 0.63, 'IN': 1.49,
          'VBN': 1.27, 'POS': 0.39, '-NONE-': 0, 'VBP': 1.31,
          'WDT': -0.23, 'JJ': 1.81, 'WP': -2.30, 'VBZ': 1.43,
          'DT': 0, 'CD': 1.18, 'MD': 1.22, 'NNPS': 0.21, 'RP': 0.89,
          '$': -30.30, 'NN': 1.70, 'RBR': 1.23, 'JJS': 0.18, ',': 0,
          '.': 0, 'VB': 3.37, 'TO': 1.73, 'PRP': 1.74, 'RB': 0.98,
          ':': -2.30, 'NNS': 1.37, 'JJR': -1.30, 'NNP': 0.41}
...
for level in range(2,6):
    ...
    for corefStory in corefStories:
        ...
        for q in range(1,6):
            [question,answer,constraints]=processQuestion(questions[q],corefre1,corefre2,corefre3,corefre4,corefre5,corefre6,weights5,locations,persons,times);
            result=getBestAnswer(question,constraints,sentences,(locations,persons,times));
            scores[level-2]=scores[level-2]+correctAnswer(result[0],answer[2:len(answer)-2]);
            scores2[level-2]=scores2[level-2]+correctAnswer(result[0],answer[2:len(answer)-2])+correctAnswer(result[1],answer[2:len(answer)-2]);
            counters[level-2]=counters[level-2]+1;
print 'Scores:',;
sc=map(lambda s, c: s*100.0/c, scores, counters);
sc2=map(lambda s, c: s*100.0/c, scores2, counters);
print sc
print 'Average:',
print sum(sc)/len(sc);
